...
with open(log_path, mode='w') as log:
	log.write("program Phase retrieval multi ver.20190315")
...
#				print("SW_ips = " + str(SW_ips))
				...
				threshold = float(SW_delta)*cp.amax(cp.real(G_dens), axis=(1,2))
...
#			print(str(n) + ", " + str(x_G) + ", " + str(y_G)) # + ", " + str(x_sum) + ", " + str(y_sum))
			...
			R_dens[n,:,:]=cp.roll(R_dens[n,:,:], int(row/2)-x_G, axis=1)
			R_dens[n,:,:]=cp.roll(R_dens[n,:,:], int(col/2)-y_G, axis=0)
	
			cp_sup[n,:,:]=cp.roll(cp_sup[n,:,:], int(row/2)-x_G, axis=1)	
			cp_sup[n,:,:]=cp.roll(cp_sup[n,:,:], int(col/2)-y_G, axis=0)
