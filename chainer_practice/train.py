...
#学習したモデルを出力するファイル名
resultFn = "C:\Python_Programs\chainer_practice\sampleNN.model"
...
epoch = 1000 #学習回数
batch = 16 #バッチサイズ
hiddens = [inputNum,300,200,len(df.iloc[:,inputNum].unique())] #各層のノード数
...
class MyChain(Chain):
    ...
    def __init__(self):
        ...
        l1=L.Linear(hiddens[0], hiddens[1]).to_gpu(),
        l2=L.Linear(hiddens[1], hiddens[2]).to_gpu(),
        l3=L.Linear(hiddens[2], hiddens[3]).to_gpu(),
    )
...
def learning(): 
    ...
    np_train_data = np.array(train_df.iloc[:, :-1].astype(np.float32))
    np_test_data = np.array(test_df.iloc[:, :-1].astype(np.float32))
    train_data = cuda.to_gpu(np_train_data, device=0)
    test_data = cuda.to_gpu(np_test_data, device=0)
 
    #目的変数もnumpy配列にする。
    np_train_target = np.array(train_df.iloc[:,inputNum]).astype(np.int32)
    np_test_target =  np.array(test_df.iloc[:,inputNum]).astype(np.int32)
    train_target = cuda.to_gpu(np_train_target, device=0)
    test_target = cuda.to_gpu(np_test_target, device=0)
    ...
    trainer.extend(extensions.PrintReport(['epoch', 'iteration', 'main/accuracy', 'validation/main/loss', 'validation/main/accuracy', 'elapsed_time'])) #レポートの内容
