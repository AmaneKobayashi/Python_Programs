...
log_path=[""]*10
for i in range(10)
	log_path[i]="log_" + str(0.1*float(i+1)) + ".csv"
...
#diffファイルの定義
diff=[[""]*len(list_dir_mrc) for in range(10)]
n=np.zeros[10]
...
for i in range(len(list_dir_mrc)):
	...
	diff[int(NOR_ex/10)+1,n[int(NOR_ex/10)+1]]=df.iat[1,2]
	n[int(NOR_ex/10)+1]=n[int(NOR_ex/10)+1]+1
