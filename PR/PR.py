...
#	exit()
	...
	if((i+1==int(iteration)+int(additional_iteration)) & (OSS_flag!=1) & (iteration!="0")):	
		...
		x_axis_sum=cp.sum(R_dens.real, axis=1)
		y_axis_sum=cp.sum(R_dens.real, axis=0)
...
#		print(str(i+1).zfill(6) + ", " + str(x_G) + ", " + str(y_G))
		...
		R_dens.real=cp.roll(R_dens.real, int(row/2)-x_G, axis=1)
		R_dens.real=cp.roll(R_dens.real, int(col/2)-y_G, axis=0)	
