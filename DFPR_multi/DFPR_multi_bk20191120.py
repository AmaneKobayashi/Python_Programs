...
if(OSS_flag!=1):
	...
	#OSS
	for i in range(n_parameter):
		if((flag_list[i]==0) & (i!=5) & (i!=6) & (i!=7) & (i!=8) & (i!=9) &(i!=10) & (i!=13) & (i!=16) & (i!=17) & (i!=20)):
			if(input_parameter==0):
				print("OSS mode")
			print("please input parameter : [" + parameter_name_list[i] + "]")	
			input_parameter=1
...
np_diff_donut=np_diff * np.square(np_donut_target)
...
dilate=cv2.dilate(np_th,d_kernel)

#d2_kernel=np.ones((int(2*Wgauss),int(2*Wgauss)), np.uint8)
#for x in range(int(2*Wgauss)):
#	for y in range(int(2*Wgauss)):
#		d=np.sqrt(np.square(x-Wgauss) + np.square(y-Wgauss))
#		if(d>=Wgauss):
#			d2_kernel[x,y]=0
#d2_kernel=np.asarray(d2_kernel, dtype=np.uint8)
...
if(target_size_flag != 1):	
	i_target_size = np.sum(erode)
	target_size = str(i_target_size)
	print("target_size = " + target_size)
if((target_size_flag==1) & (target_size=="")):
	i_target_size = np.sum(erode)
	target_size = str(i_target_size)
	print("target_size = " + target_size)
if((target_size_flag==1) & (target_size[0]=="-")):
	i_target_size = np.sum(erode)
	target_size = str(i_target_size)
	print("target_size = " + target_size)
...
with open(log_path, mode='w') as log:
	log.write("program Phase retrieval multi ver.20191023")
...
#					SW_mode="AC"
	...
	if(i+1==int(iteration)+int(additional_iteration)):
		...
		if(DFPR_flag == 1):
			R_dens_abs=cp.absolute(R_dens)*cp_sup
...
#			AC_mode="ave_std"
	...
	#
				...
				NOR[n]=np.sum(NOR_array)/float(target_size)
